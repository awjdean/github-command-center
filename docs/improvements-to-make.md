# UX Improvements

- [X] Move the "Quit" and the "Launch at login" options into Settings.

- [ ] Better UX once a token has been added.
- [X] User can't edit GitHub token after it has been entered + verified. then must remove it.
- [X] Show the user which repos are being tracked.
- [ ] Make the "Accessible repos" section in settings be collapsible. It's default state should be collapsed.
- [ ] Displayed number in the menu bar should be the number of PRs that need your attention, not the number of PRs being tracked.
- [X] Only send the user to the GitHub link for the PR when the PR number is clicked.
- [ ] Ability to reorder PRs.
- [ ] 3rd section: "Your draft PRs" containing any PRs that are drafts and that you either created or are assigned to.


# Features


Efficency
- [ ] Don't poll draft PRs.
